...
use PhonyBone::ListUtilities qw(in_list greatest);
...
BEGIN: {
    ...
    # set $options{dataset_dir} via $options{trends_home};
    my $trends_home=$options{trends_home};
    my $data_dir=File::Spec->catfile($trends_home, 'data', 'GEO', 'datasets');
    die "$data_dir: $!\n" unless -d $data_dir;
    GEO->data_dir($data_dir);
    $options{dataset_dir}=$data_dir;
    warnf "GEO data_dir: %s\n", GEO->data_dir;
...
# Find all files of interest and parse each one; 
# for each record in the file, call insert() (except database records)
sub rebuild {
...
# write the data table for the record; (currently not implemented)
# clean up _keys from record, then dispatch to appropriate insert routine
sub insert {
    my ($record, $mongos)=@_;
    my $class=ref $record;
...
# get rid keys that start with sample_, or whatever
sub delete_geo_id_keys {
    my ($geo)=@_;
    my $type=(split('::', lc ref $geo))[-1]; 
    my $regex=qr(^${type}_G\w\w\d+);	       
    my @sample_keys=grep /$regex/, keys %$geo;
    delete $geo->{$_} for @sample_keys;
}


# Store a dataset to db;
# tie the dataset's series to the ds
sub insert_dataset {
    my ($dataset, $mongos)=@_;
...
#    $ss->update({upsert=>1});
    ...
    $n_subsets=greatest($n_subsets, $nth_subset);
