...
has 'subset_phenos_hash' => (is=>'ro', isa=>'HashRef', 
			     lazy=>1, builder=>'_build_subset_phenos_hash');
...
sub is_subset_pheno {
    my ($self, $pheno)=@_;
    $self->subset_phenos_hash->{$pheno};
}
sub subset_phenos {
    my ($self)=@_;
    my @phenos=keys %{$self->subset_phenos_hash};
